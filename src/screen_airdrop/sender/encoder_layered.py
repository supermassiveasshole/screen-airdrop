...
from screen_airdrop.common.protocol_gray4 import encode_gray4_symbols
...
def _max_payload_bytes_for_body_capacity(body_capacity_bytes: int) -> int:
    low = 0
    high = int(body_capacity_bytes)
    while low < high:
        mid = (low + high + 1) // 2
        if rs_encoded_size(LAYERED_BODY_RS, layered_body_overhead_bytes() + mid) <= body_capacity_bytes:
            low = mid
        else:
            high = mid - 1
    return low
...
def build_layout_layered(
    ...
    bootstrap_coded_bits = (layered_bootstrap_payload_size_bytes() + LAYERED_BOOTSTRAP_RS.nsym) * 8
    required_control_cells = bootstrap_coded_bits + LAYERED_BOOTSTRAP_REFERENCE_CELLS
    ...
    bootstrap_cells = control_cells[:bootstrap_coded_bits]
    bootstrap_reference_cells = control_cells[
        bootstrap_coded_bits : bootstrap_coded_bits + LAYERED_BOOTSTRAP_REFERENCE_CELLS
    ]
    ...
    user_payload_capacity_bytes = _max_payload_bytes_for_body_capacity(body_capacity_bytes)
    return LayeredLayout(
        ...
        bootstrap_capacity_bits=len(bootstrap_cells),
...
def frame_capacity_bytes_layered(
    ...
    return build_layout_layered(grid_w, grid_h, guard_band, corner_size).user_payload_capacity_bytes
...
def _encode_bootstrap_bits(fields: BootstrapFields) -> List[int]:
...
def _encode_body_symbols(
    *,
    ...
    coded = encode_rs_bytes(LAYERED_BODY_RS, body_raw)
...
def build_symbol_modules_layered(
    ...
    *,
    ...
    if int(header.frame_type) == 0:
        ...
        body_coded_len = rs_encoded_size(LAYERED_BODY_RS, layered_body_overhead_bytes() + payload_len)
    ...
    if int(header.frame_type) == 0:
        ...
        body_symbols = _encode_body_symbols(body_raw=body_raw, body_mask_id=body_mask_id, layout=layout)
        for idx, (x, y) in enumerate(layout.body_coords):
...
def layered_profile_metadata() -> dict[str, int]:
    return {
        "bootstrap_profile_id": LAYERED_BOOTSTRAP_PROFILE_DEFAULT,
        "bootstrap_ecc_profile_id": LAYERED_BOOTSTRAP_RS.profile_id,
        "body_profile_id": LAYERED_BODY_PROFILE_DEFAULT,
        "body_ecc_profile_id": LAYERED_BODY_RS.profile_id,
...
def layered_control_layout_metadata() -> dict[str, int]:
    return {
