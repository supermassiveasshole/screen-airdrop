...
class PipelineRunner:
    ...
    def run(self) -> Tuple[int, Report]:
        ...
        try:
            ...
            report = self._build_aborted_report()
    ...
    def _build_aborted_report(self) -> Report:
        """Build a best-effort aborted report even if startup was interrupted."""
        try:
            ...
            report = Report()
            report.set("status", "aborted")
            report.set("output_size_bytes", 0)
            report.set("output_path", "")
            with contextlib.suppress(Exception):
                report.update(self.pipeline.snapshot())
